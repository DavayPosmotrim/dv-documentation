...
<tr>
<td>id</td><td>Серийный номер устройстав</td><td>String</td>
</tr>
<tr>
...
<tr>
<td>imgUrl</td><td>Ссылка на постер подборкм</td><td>String</td>
</tr>
</table>
...
## Получение информации о комнате ожидания (сессии) (Переодический запрос)
...
## Пометка фильма как понравившийся
## POST /like
Структура запроса RequestBody
```json
{
...
## Получение количества совпадений и последнего match фильма (Переодический запрос)
## GET /matches
1. Совпадением фильма считается когда все участники сесии пометили фильм как понравившийся
...
## Получение списка пользователей подключенных к событию "Рулетка" (Переодический запрос)
## GET /roulette_users
Структура запроса RequestBody
```json
{
